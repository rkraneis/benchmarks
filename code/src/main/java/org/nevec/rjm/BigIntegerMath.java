...
public class BigIntegerMath {
     ...
     * @author Richard J. Mathar
     */
    static public List<BigInteger> divisors(final BigInteger n) {
     ...
     * Evaluate floor(sqrt(n)).
     *
     * @param n The non-negative argument. Arguments less than zero throw an
     * ArithmeticException.
     * @return The integer square root, the square root rounded down.
     * @since 2010-08-27
     * @author Richard J. Mathar
     ...
     *
     * @param A The matrix.
     * @param r The row index of the row to be removed (0-based). An exception
     * is thrown if this is outside the range 0 to the upper row index of A.
     * @param c The column index of the column to be removed (0-based). An
     * exception is thrown if this is outside the range 0 to the upper column
     * index of A.
     ...
     * @param v The column vector to be inserted. With the current
     * implementation, it must be at least as long as the row count, and its
     * elements that exceed that count are ignored.
     ...
     *
     * @param A The square matrix. If column and row dimensions are unequal, an
     ...
     *
     * @param A The square matrix. If it is not of full rank, an
     * ArithmeticException is thrown.
     * @param rhs The right hand side. The length of this vector must match the
     ...
     *
     * @param c Represents the coefficients c[0]+c[1]*x+c[2]*x^2+.. of the
     * polynomial
     ...
     * @author Richard J. Mathar
     */
    static public BigInteger valueOf(final List<BigInteger> c, final BigInteger x) {
        if (c.isEmpty()) {
            return BigInteger.ZERO;
        }
        BigInteger res = c.get(c.size() - 1);
        for (int i = c.size() - 2; i >= 0; i--) {
            res = res.multiply(x).add(c.get(i));
        }
        return res;
    }
