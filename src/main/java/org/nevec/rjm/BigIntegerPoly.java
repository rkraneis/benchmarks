...
/**
 * Polynomial with integer coefficients. Alternatively to be interpreted as a
 * sequence which has the polynomial as an (approximate) generating function.
 *
 * @since 2010-08-27
...
public class BigIntegerPoly implements Cloneable {
    ...
    /**
     * Default ctor. Creates the polynomial p(x)=0.
     */
     ...
     */
    public BigIntegerPoly(final List<BigInteger> c) {
        a = new ArrayList<>(c);
        simplify();
    } /* ctor */
    ...
    public BigInteger valueOf(final BigInteger x) {
        ...
        BigInteger res = a.get(a.size() - 1);
        /* Heron casted form
         */
        for (int i = a.size() - 2; i >= 0; i--) {
            res = res.multiply(x).add(a.get(i));
        }
        return res;
    } /* valueOf */
     ...
     *
     * @param n the zero-based index of the coefficient. n=0 for the constant
     * term. If the polynomial has not yet the degree to need this coefficient,
     ...
     *
     * @return the number of polynomial coefficients. Differs from the
     ...
     *
     * @return power of the smallest non-zero coefficient. If the polynomial is
     * identical to 0, 0 is returned.
     */
    public int ldegree() {
        for (int n = 0; n < a.size(); n++) {
            if (a.get(n).compareTo(BigInteger.ZERO) != 0) {
                return n;
            }
        }
        return 0;
     ...
     * @return the product of this with the factor. All coefficients of this
     * have been multiplied individually by the factor.
     * @since 2010-08-27
     */
    public BigIntegerPoly multiply(final BigInteger val) {
        ...
        if (val.compareTo(BigInteger.ZERO) != 0) {
            for (int n = 0; n < a.size(); n++) {
                resul.set(n, a.get(n).multiply(val));
            }
        }
        return resul;
    } /* multiply */
     ...
     * difference of the degree of this and the degree of val. [1] the remainder
     * polynomial. This = returnvalue[0] + returnvalue[1]/val .
     * @since 2012-03-01
     */
    public BigIntegerPoly[] divideAndRemainder(final BigIntegerPoly val) {
         ...
         */
        if (valSimpl.degree() == 0 && valSimpl.a.get(0).compareTo(BigInteger.ZERO) == 0) {
        ...
        if (thisSimpl.degree() < valSimpl.degree()) {
             ...
             */
            ret[0] = new BigIntegerPoly();
            BigInteger[] newc = thisSimpl.a.get(thisSimpl.a.size() - 1).divideAndRemainder(valSimpl.a.get(valSimpl.a.size() - 1));
     ...
     *
     * @return The representation a0+a1*x+a2*x^2+... The terms with zero
     * coefficients are not mentioned.
     * @since 2008-10-26
     */
    public String toPString() {
        String str = new String();
        for (int n = 0; n < a.size(); n++) {
            final BigInteger num = a.get(n);
    ...
    /**
     * Simplify the representation. Trailing values with zero coefficients (at
     * high powers) are deleted.
     */
    protected void simplify() {
        int n = a.size() - 1;
        if (n >= 0) {
            while (a.get(n).compareTo(BigInteger.ZERO) == 0) {
                a.remove(n);
                if (--n < 0) {
                    break;
                }
    ...
    public BigIntegerPoly derive() {
        if (a.size() <= 1) {
            ...
            for (int i = 1; i <= degree(); i++) {
                final BigInteger c = a.get(i).multiply(new BigInteger("" + i));
                d.set(i - 1, c);
            }
            return d;
        }
    ...
    public BigIntegerPoly binomialTInv(int maxdeg) {
        ...
        for (int i = 0; i <= maxdeg; i++) {
            ...
            for (int j = 0; j <= i && j < a.size(); j++) {
                if ((j + i) % 2 != 0) {
                    c = c.subtract(a.get(j).multiply(BigIntegerMath.binomial(i, j)));
                } else {
                    c = c.add(a.get(j).multiply(BigIntegerMath.binomial(i, j)));
                }
            }
            r.set(i, c);
        }
    ...
    public Vector<BigInteger> iroots() {
        ...
        /* collect the zero
         */
        if (a.get(0).compareTo(BigInteger.ZERO) == 0) {
            res.add(BigInteger.ZERO);
        }

        /* collect the divisors of the constant element (or the reduced polynomial) */
        int l = ldegree();
        if (a.get(l).compareTo(BigInteger.ZERO) != 0) {
            List<BigInteger> cand = BigIntegerMath.divisors(a.get(l).abs());

            /* check the divisors (both signs) */
            for (int i = 0; i < cand.size(); i++) {
                BigInteger roo = valueOf(cand.get(i));
                if (roo.compareTo(BigInteger.ZERO) == 0) /* found a root cand[i] */ {
                    res.add(cand.get(i));
                }
                roo = valueOf(cand.get(i).negate());
                if (roo.compareTo(BigInteger.ZERO) == 0) {
                    res.add(cand.get(i).negate());
                }
            }
        }
        return res;
     ...
     * Only factors with non-zero absolute coefficient are generated. This means
     * the factors are of the form x^2+a*x+b=0 with nonzero b.
     * @since 2012-03-01
     */
    protected Vector<BigIntegerPoly> i2roots() {
        ...
        BigInteger bsco = a.get(0).abs();
        List<BigInteger> b = BigIntegerMath.divisors(bsco);
        BigInteger csco = a.get(a.size() - 1).abs();
        List<BigInteger> c = BigIntegerMath.divisors(csco);
        ...
        List<BigComplex> roo = thisDec.roots(6 + (int) (0.3 * bsco.bitCount()));
    ...
    /**
     * Factorization into integer polynomials. The current factorization detects
     * only factors which are polynomials of order up to 2.
     *
     * @return The vector of factors. Factors with higher multiplicity are
     * represented by repetition.
     * @since 2012-03-01
     */
    public List<BigIntegerPoly> ifactor() {
        /* this ought be entirely rewritten in terms of the LLL algorithm
         */
        List<BigIntegerPoly> fac = new ArrayList<>();

        /* collect integer roots (polynomial factors of degree 1) */
        List<BigInteger> r = iroots();
         ...
         */
        List<BigIntegerPoly> pol2 = i2roots();
         ...
         */
        if (res[0].degree() > 0 || res[0].a.get(0).compareTo(BigInteger.ONE) != 0) {
            fac.add(res[0]);
        }
        return fac;
    } /* ifactor */
