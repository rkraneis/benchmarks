...
public class Rational implements Cloneable, Comparable<Rational> {
     ...
     * @since 2009-05-18
     */
    static public BigInteger MAX_INT = BigInteger.valueOf(Integer.MAX_VALUE);
    static public BigInteger MIN_INT = BigInteger.valueOf(Integer.MIN_VALUE);
     ...
     *
     * @param str the string. This either has a slash in it, separating two
     * integers, or, if there is no slash, is representing the numerator with
     * implicit denominator equal to 1. Warning: this does not yet test for a
     ...
     *
     * @param str the string. This either has a slash in it, separating two
     * integers, or, if there is no slash, is just representing the numerator.
     * @param radix the number base for numerator and denominator Warning: this
    ...
    public Rational(String str, int radix) throws NumberFormatException {
        ...
        if (posSlash == -1) {
             ...
             */
            a = new BigInteger(str.substring(0, posSlash), radix);
            b = new BigInteger(str.substring(posSlash + 1), radix);
            normalize();
        }
    }
     ...
     * r-th root.
     *
     * @param r the inverse of the exponent. 2 for the square root, 3 for the
     * third root etc
     ...
     *
     * @param mc the mathematical context which determines precision, rounding
     * mode etc
