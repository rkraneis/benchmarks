...
public class BigSurdVec implements Comparable<BigSurdVec> {
    ...
    public int signum() {
         ...
         */
        int sig0 = terms.get(0).signum();
        int offsig = 1;
        for (; offsig < terms.size(); offsig++) {
            if (terms.get(offsig).signum() != sig0) {
                break;
            }
        }
         ...
         */
        if (terms.size() == 2) {
            return terms.get(0).compareTo(terms.get(1).negate());
        }
        ...
        if (terms.size() == 3) {
            BigSurdVec lhs;
            if (offsig == 2) {
                lhs = new BigSurdVec(terms.get(0), terms.get(1));
            } else {
                lhs = new BigSurdVec(terms.get(0), terms.get(2));
            }
            lhs = lhs.sqr();
            BigSurd rhs = new BigSurd(terms.get(offsig).sqr(), Rational.ONE);
            if (lhs.compareTo(lhs) > 0) /* dominating sign was t(0)+t(offbar)
             */ {
                return terms.get(0).signum();
            } else {
                return terms.get(offsig).signum();
            }
        }
    ...
    public BigDecimal BigDecimalValue(MathContext mc) {
        ...
        if (terms.isEmpty()) {
            ...
            return terms.get(0).BigDecimalValue(mc);
        }
    ...
    public BigSurdVec sqr() {
        ...
        for (BigSurd term : terms) {
            resul.terms.add(new BigSurd(term.sqr(), Rational.ONE));
        }
        for (int i = 0; i < terms.size() - 1; i++) {
            for (int j = i + 1; j < terms.size(); j++) {
                resul.terms.add(terms.get(i).multiply(terms.get(j)).multiply(2));
            }
        }
        resul.normalize();
        return resul;
